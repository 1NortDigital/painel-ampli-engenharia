...
// ============================================================
import {
  CLIENT, PIPELINES, ORIGINS, PRODUCTS, deriveOrigins, deriveProducts, WINDOW_LABELS, PROFILE_FIELDS,
...
async function fetchSyncTime() {
  ...
  if (!elSync) return;
    // ⚠️ TROCAR: nome do repo do cliente
  try {
    const url = "https://api.github.com/repos/1NortDigital/painel-ampli-engenharia/commits?path=leads.json&page=1&per_page=1";
...
function render() {
  ...
  addFilter("Origem", multiSelect("origin", "Todas", deriveOrigins(LEADS).map((o) => ({ value: o, label: o }))));
  addFilter("Produto", multiSelect("product", "Todos", deriveProducts(LEADS).map((p) => ({ value: p, label: p }))));
