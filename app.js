// ============================================================
// 1NORT — Painel Ampli Engenharia (Kommo)
// conta: limpezaampliengenhariaorg  ·  account_id 34044199
// ============================================================

export const CLIENT = {
  name: "Ampli Engenharia",
  eyebrow: "",
  subtitle: "",
  crmLeadUrl: (id) => `https://limpezaampliengenhariaorg.kommo.com/leads/detail/${id}`,
};

export const PIPELINES = [
  { id: 10894675, label: "Pré-Qualificação", isPrimary: true },
  { id: 10340975, label: "Funil de vendas" },
  { id: 14078508, label: "Funil de Resgate" },
  { id: 10894699, label: "Descarte" },
];

// Kommo: 142 = ganho, 143 = perdido em TODOS os pipelines.
export const WON_STAGE = 142;
export const LOST_STAGE = 143;

// --- Funil de vendas (10340975), na ordem de sort ---
// 83546819 Contato Realizado (10)
// 79316387 Qualificado (20)
// 108675948 fazer orçamento (30)
// 108673596 proposta agendada (40)
// 108673600 No-show [7D] (50)
// 108675536 pre-orçamento enviado (60)
// 108673604 Proposta Apresentada (70)
// 79298291 não teve retorno (80)
// 79298295 visita agendada (90)
// 108673608 Não teve visita [remarcar] (100)
// 108673612 negociação (110)

// "Qualificado ou além"
export const QUALIFIED_STAGES = [
  79316387, 108675948, 108673596, 108673600, 108675536,
  108673604, 79298291, 79298295, 108673608, 108673612, 142,
];

// "Proposta agendada ou além"
export const PROPOSAL_STAGES = [
  108673596, 108673600, 108675536, 108673604, 79298291,
  79298295, 108673608, 108673612, 142,
];

// "Visita agendada ou além"
export const VISIT_STAGES = [79298295, 108673608, 108673612, 142];

export const ORIGINS = ["[Meta] Whatsapp", "Indicação", "[Meta] Cadastro", "Loja", "Sem origem"];
export const PRODUCTS = ["Residencial", "Comercial", "Sem produto"];

// Deriva a lista de origens/produtos a partir dos dados reais (funciona com
// qualquer CRM/conta, sem depender de valores fixos). Ordena por frequência.
export function deriveOrigins(leads) {
  const count = new Map();
  leads.forEach((l) => {
    const o = (l.origin ?? "").trim() || "Sem origem";
    count.set(o, (count.get(o) || 0) + 1);
  });
  return [...count.entries()].sort((a, b) => b[1] - a[1]).map((e) => e[0]);
}
export function deriveProducts(leads) {
  const count = new Map();
  leads.forEach((l) => {
    const p = (l.product ?? "").trim() || "Sem produto";
...
export function applyFilter(leads, f) {
  return leads.filter((l) => {
    ...
    if (f.origin.length && !f.origin.includes((l.origin ?? "").trim() || "Sem origem")) return false;
    if (f.product.length && !f.product.includes((l.product ?? "").trim() || "Sem produto")) return false;
    return true;
  });
}
...
export function funnelStages(scope, prev) {
  ...
  // "proposta agendada ou além" (inclui No-show, que já foi agendado)
  const reachedScheduled = (l) => l.status === "won" || !!l.proposalAt || [
    108673596, 108673600, 108675536, 108673604, 79298291,
    79298295, 108673608, 108673612, 142,
  ].includes(l.stageId);
  // "Proposta Apresentada ou além" (inclui 'não teve retorno', que é pós-apresentação)
  const reachedPresented = (l) => l.status === "won" || [
    108673604, 79298291, 79298295, 108673608, 108673612, 142,
  ].includes(l.stageId);
...
export function discardBreakdown(scope) {
  const STAGES = [
    { id: 83547063, label: "Lead Ruim" },
    { id: 108673584, label: "Fora da localização" },
  ];
